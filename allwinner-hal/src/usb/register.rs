...
/// rx_fifo_address are indexed by index. The chip-specific PHY is a separate
/// MMIO peripheral represented by [`PhyRegisterBlock`].
#[repr(C)]
...
/// Independent USB PHY peripheral used by the V821 BootROM initialization sequence.
#[repr(C)]
pub struct PhyRegisterBlock {
    ...
    pub phy_control_28nm: RW<PhyControl28nm>,
...
/// USB PHY 28 nm and VC-bus control.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, Default)]
#[repr(transparent)]
pub struct PhyControl28nm(u32);
...
mod tests {
    ...
    #[test]
    fn phy_controls_match_the_bootrom_sequence() {
        let control = PhyControl28nm(u32::MAX).enable_vc_bus().prepare_vc_write();
        ...
        assert_eq!(PhyControl28nm(u32::MAX).power_up().0 & (1 << 3), 0);
    ...
    fn specialized_accessors_write_exact_typed_values() {
        ...
        let mut interface_backing = InterfaceStatusControl(0x75);
        // SAFETY: same transparent-wrapper layout argument as above.
        let interface = unsafe {
            &*((&mut interface_backing as *mut InterfaceStatusControl)
                .cast::<InterfaceStatusControlRegister>())
        };
        interface.modify_control(InterfaceStatusControl::force_id_high);
        assert_eq!(interface_backing.0, 0x0000_c005);
