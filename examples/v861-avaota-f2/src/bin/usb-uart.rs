...
fn main(peripherals: Peripherals, clocks: Clocks) {
    ...
    let strings = [StringDescriptors::default()
        ...
        .product("V861 USB UART")
        .serial_number("V861-AVAOTA-F2")];
    let mut usb_device = UsbDeviceBuilder::new(&usb_bus, UsbVidPid(0x1f3a, 0x8610))
        .strings(&strings)
        .unwrap()
    ...
    loop {
        ...
        if !greeting_visible {
            if !write_all(
                &mut usb_device,
                &mut serial,
                b"Welcome to Allwinner-HAL v861-avaota-f2 example!\r\n> ",
            ) {
                continue;
            }
                ...
                InputEvent::Command(command) => {
                    ...
                    let response = match command {
                        ...
                        Command::Exit => {
                            ...
                            delay.delay_ms(10);
                            allwinner_rt::soc::v861::reset_to_fel();
                        }
